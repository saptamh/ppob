...
<div class="container-fluid">
    <h1 class="h3 mb-2 text-gray-800">KPI (Key Performance Indicators)</h1>
    @can('kpi-create')
    <div class="row">
        <div class="col-lg-12">
            <a href="{{ route('kpi.add') }}" class="btn btn-primary btn-circle">
...
$(document).ready(function() {
    var t = $('#DataTable').DataTable({
        ...
        },
        {
            ...
            searchable: false,
            sortable: false,
            defaultContent: "<center>@can('kpi-edit')<button class='btn btn-warning btn-sm' id='edit_btn'>Edit</button>@endcan " +
                "@can('kpi-delete')<button class='btn btn-danger btn-sm' id='remove_btn'>Delete</button>@endcan</center>"
        }],
        columns: [
            {data: "id"},
            {data: "start_date"},
            ...
            {data: "job_percentage", render: function(data, type, row) {
                return Math.ceil(data);
            }},
            {data: "quality_percentage", render: function(data, type, row) {
                return Math.ceil(data);
            }},
            {data: "attitude_percentage", render: function(data, type, row) {
                return Math.ceil(data);
            }},
    ...
    $('#DataTable tbody').on('click', '#edit_btn', function () {
        ...
        window.location.href = baseUrl + "/kpi/edit/" + data_row.id;
    });
    ...
    $('#DataTable tbody').on('click', '#remove_btn', function () {
        ...
        if (c) {
            ...
            $.ajax({
                url: baseUrl + "/kpi/destroy/" + data_row.id,
                method: 'delete',
